...
**Real reader as of 2026-09-01**: `features/training/services/fetchWorkoutHistory.ts` — Workout Detail's HISTORY section, one workout's own events (`eq('workout_id', ...)`), newest first. Reads `id`/`summary`/`reasoning`/`created_at`; `trigger_type` needs no display mapping since `summary` is already the athlete-facing sentence. `reasoning` renders too (shown only when present) — the engine is the first writer to ever set it to something real (a longer explanation of its own trim), manual callers still leave it `null`, so their history rows render as a single line.
